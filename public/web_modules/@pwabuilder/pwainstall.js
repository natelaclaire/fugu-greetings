        ...
        if ((a++, 1 === t.nodeType)) {
          ...
          if (t.data === r) {
            ...
            (null !== t.previousSibling && a !== o) ||
              ...
              (o = a),
              this.parts.push({ type: "node", index: a }),
              null === t.nextSibling ? (t.data = "") : (i.push(t), a--),
              d++;
          } else {
            let e = -1;
...
class x {
  ...
  _getValue() {
    ...
    for (let n = 0; n < e; n++) {
      ...
      if (void 0 !== e) {
        const t = e.value;
        if (y(t) || !v(t)) i += "string" == typeof t ? t : String(t);
...
class b {
  ...
  __commitTemplateResult(t) {
    ...
    if (this.value instanceof m && this.value.template === e)
      ...
      const i = new m(e, t.processor, this.options),
        n = i._clone();
...
const R = (t) => (e) => {
    ...
    if (((s = n.keyString.get(o)), void 0 === s)) {
      ...
      j && window.ShadyCSS.prepareTemplateDom(i, t),
        (s = new d(e, i)),
        n.keyString.set(o, s);
    }
      ...
      ? (function (t, e, i = null) {
          ...
          for (; o.nextNode(); ) {
            for (
              l++,
                o.currentNode === i &&
                  ((a = M(e)), i.parentNode.insertBefore(e, i));
              -1 !== r && s[r].index === l;

            ) {
              if (a > 0) {
                for (; -1 !== r; ) (s[r].index += a), (r = O(s, r));
                return;
              }
              r = O(s, r);
            }
...
let it = class extends tt {
  constructor() {
    super(),
      ...
      window.addEventListener("beforeinstallprompt", (t) =>
        this.handleInstallPromptEvent(t)
      ),
  ...
  render() {
    return B`${
      !0 !== this.usecustom && this.shouldShowInstall() && !1 === this.installed
        ? B`<button part=openButton id=openButton @click=${() =>
            this.openPrompt()}><slot>${this.installbuttontext}</slot></button>`
        : null
    } ${
      !0 === this.openmodal
        ? B`<div id=installModalWrapper>${
            this.openmodal
              ? B`<div id=background @click=${() => this.cancel()}></div>`
              : null
          }<div id=installModal part=installModal><div id=headerContainer><div id=logoContainer><img src=${
            this.iconpath ? this.iconpath : this.manifestdata.icons[0].src
          } alt="App Logo"><div id=installTitle><h1>${
            this.manifestdata.short_name || this.manifestdata.name
          }</h1><p id=desc>${
            this.explainer
          }</p></div></div><button id=closeButton @click=${() =>
            ...
            this.manifestdata.features
              ? B`<div id=keyFeatures><h3>${this.featuresheader}</h3><ul>${
                  this.manifestdata.features
                    ? this.manifestdata.features.map((t) => B`<li>${t}</li>`)
                    : null
                }</ul></div>`
              : null
          } ${
            this.manifestdata.screenshots
              ? B`<div id=screenshotsContainer><button @click=${() =>
                  this.scrollToLeft()}><svg xmlns=http://www.w3.org/2000/svg viewBox="0 0 512 512"><path d="M401.4 224h-214l83-79.4c11.9-12.5 11.9-32.7 0-45.2s-31.2-12.5-43.2 0L89 233.4c-6 5.8-9 13.7-9 22.4v.4c0 8.7 3 16.6 9 22.4l138.1 134c12 12.5 31.3 12.5 43.2 0 11.9-12.5 11.9-32.7 0-45.2l-83-79.4h214c16.9 0 30.6-14.3 30.6-32 .1-18-13.6-32-30.5-32z"/></svg></button><section id=screenshots>${this.manifestdata.screenshots.map(
                  (t) => B`<div><img alt="App Screenshot" src=${t.src}></div>`
              ...
              : null
          }</div><div id=descriptionWrapper><h3>${
            this.descriptionheader
          }</h3><p>${this.manifestdata.description}</p></div></div>${
            this.isIOS
              ? B`<p id=iosText>${this.iosinstallinfotext}</p>`
              : B`<div id=buttonsContainer>${
                  this.deferredprompt
                    ? B`<button id=installButton @click=${() =>
                        this.install()}>${this.installbuttontext} ${
